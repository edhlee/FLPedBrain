...
def conv3d_bn(x,
    ...
    if use_bn:
        ...
        #x = InstanceNormalization(axis=3, center=True, epsilon=1e-5, name=bn_name+"instance")(x)
        #x = LayerNormalization(axis=bn_axis, name=bn_name+"layernorm")(x)
        #x = tf.keras.layers.GroupNormalization(groups=32, axis=-1, name=bn_name+"group")(x)


        # instance norm
        num_channels = x.shape[-1]  # number of channels in your input
#        x = tf.keras.layers.GroupNormalization(groups=num_channels, axis=-1, name=bn_name + "instance")(x)
...
#             x = Activation('softmax', name='prediction', dtype='float32')(x)
    else:
        h = int(x.shape[2])
        w = int(x.shape[3])
